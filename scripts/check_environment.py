...
def run_docx_env(skill_dir: Path) -> tuple[bool, str]:
    env_script = skill_dir / "tools/docx/env_check.sh"
    if not env_script.exists():
        return False, "optional DOCX validation helper not bundled"
    try:
...
def check_environment(skill_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "recommendation": (
            ...
            if docx_ready
            else "完整 DOCX OpenXML 环境未就绪。可以继续使用兜底 DOCX 生成；如需更规范的 Word 结构和校验，请在项目中接入可用的 DOCX 校验工具。"
        ),
        "install_prompt": (
