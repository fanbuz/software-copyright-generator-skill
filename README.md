...
- **确认模式与偏好复用**：支持逐项确认（默认）或经申请人授权后按默认直走两种模式；流程中的选择（确认模式、截图方式、服务地址等）保存为偏好配置，下次生成时可直接沿用。
...
确认节点默认逐项停住等待申请人确认（`interactive` 模式）。申请人也可以选择按默认直走（`auto` 模式）：

```bash
python3 <skill-dir>/scripts/confirmation_preferences.py --workdir 软件著作权申请资料 --set-mode auto
python3 <skill-dir>/scripts/confirmation_preferences.py --workdir 软件著作权申请资料 --apply-defaults
```

`auto` 模式自动补齐可默认的确认动作（环境、截图方式、各草稿确认等）；登记数据不能被默认代替——申请表仍有"待用户确认"字段或草稿缺失时，对应节点保持待办并列入报告。

流程中的选择会自动保存到 `软件著作权申请资料/用户偏好.json`，下次生成时可查看并沿用：

```bash
python3 <skill-dir>/scripts/confirmation_preferences.py --workdir 软件著作权申请资料 --show
```
