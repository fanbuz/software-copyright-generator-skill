...
1. 环境检查：运行 `scripts/check_environment.py --out-dir 软件著作权申请资料`，确认 Markdown、TXT、基础 DOCX 和可选 OpenXML 校验能力。
2. 定位项目：扫描当前目录，避开依赖、构建产物、输出目录和隐藏目录；多个候选项目时询问用户。
3. 项目分析：运行 `scripts/analyze_project.py --project <项目目录> --out 软件著作权申请资料/analysis/project.json`。
4. 业务理解：用 `scripts/generate_business_context.py` 收集证据；模型阅读 README、PRD、路由、页面、接口和必要源码后写出业务理解 JSON，再生成 `草稿/业务理解.md/json` 并等待确认。
5. 登记字段：确认软件全称、版本号、著作权人、开发完成日期、首次发表状态、硬件环境、操作系统、开发工具、运行支撑环境、软件分类和技术特点。
6. 代码选择：运行 `scripts/propose_code_selection.py` 生成候选清单；模型按真实功能和运行逻辑选择源码文件，可填写 `start_line/end_line` 指定行段，写明理由并等待用户确认。
7. 草稿生成：运行 `scripts/extract_code_material.py`、`scripts/generate_application_info.py`、`scripts/generate_manual_draft.py`，生成代码、申请表信息和操作手册 Markdown。
8. 截图处理：按用户选择获取或整理截图；如果暂不截图，手册保留清晰可见的截图预留文字。
9. Markdown 确认：用户确认全部草稿后，记录 `markdown` 门禁。
10. 正式生成：运行 `scripts/build_docx_from_md.py --workdir 软件著作权申请资料 --software-name "<软件全称>" --version "<版本号>"`。
11. 最终复核：按 `references/final_review.md` 检查目标目录中的最终文件。
