...
- 用户提出生成软著材料时，先输出前置采集表并等待用户填写；表单必须按登记权属、开发发表、项目形态、业务手册、源码材料、运行截图、模板输出和正式资料检查梳理，不只询问少数字段。
- 先生成 Markdown 草稿，用户确认后再生成 Word/TXT。
- 正式资料只写入 `软件著作权申请资料/正式资料/`。
- 代码材料必须来自真实源码，禁止编造代码。
- 截图必须来自真实系统页面或用户提供图片；不得用生成图片冒充系统截图。
- 浏览器自动截图目前仅支持 Web 端服务；移动端、小程序、桌面端项目的截图由用户提供。
...
- `interactive`（默认）：逐项确认，每个门禁停住等待用户输入。
- `auto`：用户授权按默认与已保存偏好直走；运行 `scripts/confirmation_preferences.py --set-mode auto` 记录后，用 `--apply-defaults` 自动补齐可默认的门禁（环境、截图方式、业务理解确认、代码选择确认、申请表字段确认、Markdown 确认的“确认动作”）。登记数据类内容不能被默认代替：申请表仍有“待用户确认”字段、草稿缺失或源码选择缺少理由时，对应门禁保持待办并提示用户。

流程中的用户选择（确认模式、截图方式、服务地址等）自动保存到 `<workdir>/用户偏好.json`。下次在同一工作目录运行时，先执行 `scripts/confirmation_preferences.py --show` 向用户展示已保存偏好，询问是否沿用，再决定确认模式。

## 强制门禁

凡是涉及用户选择、确认或补充信息的阶段，必须停住等待用户输入；用户已通过 `auto` 模式授权的可默认门禁除外。收到确认后，用脚本记录门禁再继续：
...
- `preflight`：用户填写软件全称、版本号、著作权人、开发完成日期、发表状态、仓库模式、代码位置、业务范围、源码材料策略、截图方式、模板输出和正式资料检查词等前置信息。
...
- `screenshot-ready`：选择浏览器自动截图后，提示用户启动 Web 服务；用户确认访问地址可达并通过探测后记录门禁（自动截图仅支持 Web 端服务）。
- `markdown`：全部 Markdown 草稿完成后，用户确认可以生成正式 Word/TXT。

## 工作流

1. 前置采集：运行 `scripts/generate_input_form.py` 或 `scripts/run_stage.py --stage preflight`，向用户展示 `【请用户输入】` 表单。表单按八组收集登记与权属、开发与发表、项目形态与仓库、业务范围与用户手册、源码材料、运行与截图、模板与输出、正式资料检查；用户提交后记录 `preflight` 门禁。
...
9. 截图处理：按用户选择获取或整理截图。选择浏览器自动截图时，先运行 `scripts/capture_screenshots.py --check-only --base-url <访问地址>` 做就绪检查；服务未启动时停住，提示用户启动 Web 服务，用户确认后用 `confirm_stage.py --stage screenshot-ready --base-url <访问地址>` 记录门禁再采集（仅支持 Web 端服务）。如果暂不截图，手册保留清晰可见的截图预留文字。
...
- `scripts/confirmation_preferences.py`：确认模式设置、偏好记录与 auto 模式默认门禁补齐。
